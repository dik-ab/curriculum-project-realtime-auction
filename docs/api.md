...
- `signup` / `login` 以外の**すべてのAPIは認証必須**です(社内サービスのためゲスト閲覧はありません)。未認証は `401`(code: `UNAUTHORIZED`)。
...
| `price_updated` | server → client | room(`listing:12`)全員 | 入札成立時 | `{"listing_id": 12, "current_price": 3100, "bids_count": 2, "version": 5, "bid": {"id": 501, "bidder_name": "佐藤", "amount": 3100, "created_at": "2026-07-06T09:00:00Z"}}` |
...
| `auction_extended` | server → client | room全員 | 自動延長が発生したとき | `{"listing_id": 12, "ends_at": "2026-07-08T09:03:00Z", "extended_count": 1, "version": 6}` |
| `auction_ended` | server → client | room全員 | 締切処理・即決で確定したとき | `{"listing_id": 12, "result": "sold", "final_price": 3100, "winner_id": 8, "winner_name": "佐藤", "trade_id": 3, "version": 9}`(流札は `{"listing_id": 12, "result": "unsold", "final_price": null, "winner_id": null, "winner_name": null, "trade_id": null, "version": 9}`) |
