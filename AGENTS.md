...
このリポジトリは、社内リアルタイムオークション「Lumina Market」のリポジトリです。あなた(AIエージェント)の役割は、
`docs/` の仕様に**厳密に従って**、issue単位の実装・テスト・レビューを支援することです。

## リポジトリの性質

- このリポジトリには**2つの顔**があります。テンプレート元(dik-ab/curriculum-project-realtime-auction)は仕様書のみのリポジトリで、
  実装コードはコミットされません。一方、受講者が「Use this template」から作ったリポジトリでは、
  **このリポジトリ自体に実装コードを追加していきます**(docs/ の仕様はそのまま残し、実装と共存させます)
- AIとして作業する場合: リモートが dik-ab/ 以外なら実装リポジトリとして振る舞い、実装・テスト・コミットを普通に行ってください
...
- 小さな依存ライブラリの追加は自由です(PRの変更内容に何のために入れたかを1行書いてください)
